...
# Paring code to backup current f5_xc_dns_lb configurations, from collection of current files
...
def f5_xc_backup_configs(f5_xc_json:str, f5xc_object_type:str) -> None:
    """
    Takes JSON Configuraiton file , connects to F5 DC (XC) SaaS Platform via REST API to backup current config for that definition
    """
    ...
    if f5xc_object_type == "test":
        ...
        with open(f5_xc_json, 'r') as input_file:
            ...
            if f5xc_object_type == "1_healthcheck":
                # Step 1 - Backup healthchecks
                print(f" F5 XC DNS - Checking Health Monitor - GET for: {f5xc_object_name}")
                json_dict = f5_xc_backup_dns_lb_healthcheck(f5xc_object_name)
                backup_dict = {}
                backup_dict["metadata"] = json_dict["metadata"]
                backup_dict["spec"] = json_dict["spec"]
                json_string = json.dumps(backup_dict, indent=4)
                with open('./xc_backups/1_healthcheck/'+f5xc_object_name+'_backup.json', 'w') as output_file:
                ...
                json_dict = f5_xc_backup_dns_lb_pool(f5xc_object_name)
                backup_dict = {}
                backup_dict["metadata"] = json_dict["metadata"]
                backup_dict["spec"] = json_dict["spec"]
                json_string = json.dumps(backup_dict, indent=4)
                with open('./xc_backups/2_pool/'+f5xc_object_name+'_backup.json', 'w') as output_file:
                    output_file.write(json_string)
                time.sleep(2)

            elif f5xc_object_type == "3_dnslb":
                # Step 3 - Backup DNS LB Instances
                print(f" F5 XC DNS - Checking DNS LB - GET for: {f5xc_object_name}")
                json_dict = f5_xc_backup_dns_lb_dnslb(f5xc_object_name)
                backup_dict = {}
                backup_dict["metadata"] = json_dict["metadata"]
                backup_dict["spec"] = json_dict["spec"]
                json_string = json.dumps(backup_dict, indent=4)
                with open('./xc_backups/3_dnslb/'+f5xc_object_name+'_backup.json', 'w') as output_file:
                    output_file.write(json_string)
                time.sleep(2)
                ...
                json_dict = f5_xc_backup_dns_lb_rrecord(f5xc_dns_zone_name, f5xc_dns_zone_rr_group, f5xc_dns_zone_rr_record_name, f5xc_dns_zone_rr_get_type)
                # No need to parse out - backup entire response
                json_string = json.dumps(json_dict, indent=4)
                with open('./xc_backups/4_records/'+f5xc_dns_zone_rr_record_name+'_backup.json', 'w') as output_file:
                    output_file.write(json_string)
                time.sleep(2)
...
def f5_xc_backup_dns_lb_healthcheck(f5xc_object_name:str) -> dict:
    """
...
def f5_xc_backup_dns_lb_pool(f5xc_object_name:str) -> dict:
    """
...
def f5_xc_backup_dns_lb_dnslb(f5xc_object_name:str) -> dict:
    """
    Checks XC DNS-LB Pool
    Takes configuration json
    Expects 'f5xc_tennant_short_id' and 'f5xc_namespace' strings to be defined
...
def f5_xc_backup_dns_lb_rrecord(f5xc_dns_zone_name:str, f5xc_dns_zone_rr_group:str, f5xc_dns_zone_rr_record_name:str, f5xc_dns_zone_rr_get_type:str) -> dict:
    """
    Checks XC DNS-LB Pool
    Takes configuration json
    Expects 'f5xc_tennant_short_id' and 'f5xc_namespace' strings to be defined
...
def f5_xc_backup_dns_zones() -> str:
    """
...
if __name__ == "__main__":
    # Assign directory to parse , and backup configurations
    ...
    try:
        ...
        #                 if file_name[-5:] == ".json":
        #                     f5_xc_backup_configs(file_contents, f5xc_object_type_chosen)
        ...
        if (f5xc_object_type_chosen == "test"):
            f5_xc_backup_configs("test", "test")
                ...
                for file_name in os.listdir(folder_name):
                    ...
                    if os.path.isfile(file_contents):
                        try:
                            # Only try to parse file if it is name ends with .json
                            if file_name[-5:] == ".json":
                                f5_xc_backup_configs(file_contents, "1_healthcheck")
                ...
                for file_name in os.listdir(folder_name):
                    ...
                    if os.path.isfile(file_contents):
                        try:
                            # Only try to parse file if it is name ends with .json
                            if file_name[-5:] == ".json":
                                f5_xc_backup_configs(file_contents, "2_pool")
                ...
                for file_name in os.listdir(folder_name):
                    ...
                    if os.path.isfile(file_contents):
                        try:
                            # Only try to parse file if it is name ends with .json
                            if file_name[-5:] == ".json":
                                f5_xc_backup_configs(file_contents, "3_dnslb")
                ...
                for file_name in os.listdir(folder_name):
                    ...
                    if os.path.isfile(file_contents):
                        try:
                            # Only try to parse file if it is name ends with .json
                            if file_name[-5:] == ".json":
                                f5_xc_backup_configs(file_contents, "4_records")

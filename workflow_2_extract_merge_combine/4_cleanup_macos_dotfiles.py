...
def clean_macos_dot_files(base_foldername:str='host_ucs_v15_updated') -> None:
    ...
    if config_files_sting != "":
        ...
        sub_command = f'find "{base_foldername}" -name "._*" -delete'
    ...
    if config_files_sting != "":
        ...
        sub_command = f'find "{base_foldername}" -name ".DS_Store" -delete'
        subprocess.run(sub_command, shell=True)
    else:
...
if __name__ == "__main__":
    # assign directory
    directory = input('Please enter folder name to remove from within recursively (HINT: may navigate back a folder with ../FOLDERNAME )\n')

    # Remove all dot files recursively found under directory

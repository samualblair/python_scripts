...
def archive_ucs(bigip_conf_folder:str,file_extension_length:int=9) -> str:
    ...
    # Create Archive
    sub_command = f'find "{bigip_conf_folder}"/ -type f -o -type l -o -type d | sed s,^"{bigip_conf_folder}"/,, | grep -v -E "\\._|\\.DS_Store" | tar --disable-copyfile -czf "{path_of_new_ucs_file}" --no-recursion -C "{bigip_conf_folder}"/ -T -'
    subprocess.run(sub_command, shell=True)

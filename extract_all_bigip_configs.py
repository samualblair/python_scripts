...
#    subprocess.run(subcomamnd)
    ...
    config_files_sting_byte = subprocess.Popen(f'tar -tzf "{bigip_conf_filename}"| grep -E "bigip.conf|bigip_base.conf" | grep -v -E ".diffVersions|.bak|openvswitch"', shell=True, stdout=subprocess.PIPE).stdout.read()
    ...
    try:
        os.mkdir(f'unpacked_{bigip_conf_filename[2:len(bigip_conf_filename)]}')
    except FileExistsError:
        print('Folder already existed')

    for config_tar_file_path in string_list:
        sub_comamnd = f'tar -xzf {bigip_conf_filename} -C "unpacked_{bigip_conf_filename[2:len(bigip_conf_filename)]}" "{config_tar_file_path}"'
